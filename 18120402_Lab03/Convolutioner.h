...
{
private:
	vector<double> _kernel;// kernel vector is used to do convolution
	int _kernelWidth;	  // kernel width
	int _kernelHeight;    // kernel height

public:
	// Get vector kernel
	vector<double> GetKernel() { return _kernel; };

	// Set kernel with parameters: kernel vector, kernel witdth, kernel height
	void setKernel(vector<double> kernel, int kWidth, int kHeight);
